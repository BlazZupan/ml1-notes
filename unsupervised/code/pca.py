...
for epoch in range(n_epochs):
    ...
    loss = variance + 10.0 * orthogonality_penalty
    ...
    if (epoch + 1) % 100 == 0:
        print(f'Epoch [{epoch+1}/{n_epochs}], Variance: {-variance.item():.4f}, Orthogonality: {orthogonality_penalty.item():.4f}')
...
with torch.no_grad():
    for pc in range(2):  # For first and second principal component
        ...
        # Print results
        print(f"\nTop {n_features} features for principal component {pc+1}:")
        print("Feature name".ljust(20), "Weight")
        print("-" * 30)
